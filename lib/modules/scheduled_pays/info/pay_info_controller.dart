...
class PayInfoController {
  ...
  Future<List<RelatedRecord>> updateLastRecordIfExistAndAccount({
    int? scheduledPayId,
    int? recordId,
    bool? paid,
    DateTime? datetime,
    ...
    if (recordId != null) {
      dao.updateRecord(
        ...
        model.Record(
          ...
          datePaid: datetime,
          amount: amount, 
          balance: accountId != null && (paid ?? false) ? await dao.updateAccountBalance(accountId, amount, substract: isExpense) : null,
        ).toCleanMap()
      );
    }
